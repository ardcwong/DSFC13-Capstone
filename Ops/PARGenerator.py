...
def save_html_content_and_update_tag(spreadsheet, reference_number, html_content, report_intro, styled_table_html, feedback_section):
    ...
    if cell:
        ...
        for i, feedback in enumerate(feedback_section):
            worksheet.update_cell(cell.row, worksheet.find(f"FEEDBACK_SECTION_{i+1}").col, feedback)
...
with tab2:
    ...
    if pf_rn_y != []:
        ...
        with column__1:
            ...
            if pdf:
                st.download_button(label=f"Download PDF (**{reference_number_ops_view}**)", data=pdf, file_name=f"{reference_number_ops_view} PAR.pdf", mime="application/pdf", use_container_width = True, disabled = download_disabled, help = "Download PAR")
            else:
        ...
        st.info("Nothing to view. Generate a PAR and save it.")
